...
class FileVersionInfo(object):
    ...
    def get_version(self) -> str:
        ...
        :return:
            The version as a string, or an empty string if there is no
            match to the magic_line.
        """
...
version_objects = [
    FileVersionInfo(
        key_name='Jekyll Config',
        file_path=join(curr_location, '../_config.yml'),
        magic_line='version: ',
        strip_end_chars=1,
    ),
    FileVersionInfo(
        key_name='npm package.json',
        file_path=join(curr_location, '../package.json'),
        magic_line='  "version": "',
        strip_end_chars=3,
    ),
]
